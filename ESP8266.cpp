...
ESP8266::ESP8266(SoftwareSerial &uart, uint32_t baud) :
		m_puart(&uart)
{
...
bool ESP8266::autoSetBaud(uint32_t baudRateSet)
{
	...
	long time0 = millis();
	long baudRateArray[] =
	{ 9600, 19200, 57600, 115200 }; //These are the optional default baudrates
	...
	{ //attempt to connect to esp over each baudrate
		for (int i = 0; i < sizeof(baudRateArray); i++) //check for current esp baudrate
		{
			...
			while (m_puart->available())
			{
				...
				{ //if OK received, this is the current baudrate of the ESP
					baudFlag = 1;
					delay(15);
					break;
				}
...
String ESP8266::getLocalIP(void)
{
	...
	while (m_puart->available() > 0)
	{
		...
		{
			delay(100);
			return (inData.substring(inData.indexOf("IP") + 4,
					inData.length() - 2));
		}
	}
	return(F("Couldn't get IP adress"));
...
bool ESP8266::registerUDP(String remote_addr, uint16_t remote_port,
		uint16_t local_port)
{
	return sATCIPSTARTSingle("UDP", remote_addr, remote_port, local_port);
}
...
bool ESP8266::eATPING(String url)
{
	String data;
	rx_empty();
...
bool ESP8266::sATCIPSTARTSingle(String type, String remote_addr,
