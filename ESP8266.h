...
class ESP8266 {
 public:
     ...
     * @retval false - failure.
     */
    bool registerUDP(String remote_addr, uint16_t remote_port, uint16_t local_port);
 ...
 private:
    ...
    bool eATCWQAP(void);
    bool eATPING(String url);
    bool sATCWSAP(String ssid, String pwd, uint8_t chl, uint8_t ecn);
    bool eATCWLIF(String &list);
    
    bool eATCIPSTATUS(String &list);
    bool sATCIPSTARTSingle(String type, String remote_addr, uint16_t remote_port, uint16_t local_port = 0);
